...
class BackupEngine {
    constructor(name, encrypt, files) {
        ...
        this.destination = `${config.get('destination.current')}/${this.metadata.name}`;
    ...
    async _saveBlock(block) {
        ...
        try {
            let exists = await direxists(this.destination);
            if(!exists){
                await mkdir(this.destination);
            }
            await writefile(`${this.destination}/${block.id}.br`, block.buffer.data);
        } catch (err) {
            console.log(err);
        }
    }
